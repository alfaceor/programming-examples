...
int main(const int argc, char** argv){
  ...
  //printf("hello\n");
  // Output file
  FILE *outfile = fopen("rw2Dtrials.dat", "w");
  ...
  for (int nt=0; nt < Ntrials; nt++){
    ...
    for (int ns=0; ns < Nsteps; ns++){
      ...
      if (rnd < 0.25){
        if ( net[x+1][y] == 0) { x++; } else ns--;
      }else if ( rnd < 0.5){
        if ( net[x-1][y] == 0) { x--; } else ns--;
      }else if ( rnd < 0.75){
        if ( net[x][y+1] == 0) { y++; } else ns--;
      }else{
        if ( net[x][y-1] == 0) { y--; } else ns--;
      }
    }
    ...
    y_mean_tmp = y_mean/float(nt);

    x_var_tmp = (x_var /float(nt)- x_mean_tmp*x_mean_tmp);
    y_var_tmp = (y_var /float(nt)- y_mean_tmp*y_mean_tmp);

    //printf("%d %f %f %d\n", n, x_mean_tmp, x_var_tmp, x);
    if (nt%1000 == 0)
      fprintf(outfile, "%d %f %f %f %f %f %d %d\n", nt, x_mean_tmp, y_mean_tmp, x_var_tmp, y_var_tmp, x_var_tmp + y_var_tmp,  x, y);
  }
