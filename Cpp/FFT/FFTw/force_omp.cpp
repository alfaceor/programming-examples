...
void calc_ForceFFTw(double *force, double *cosq, double *sinq, fftw_complex *Lambda, const int N){
  ...
  // for (int i=0; i<N; i++){
  //   for (int j=0; j<N; j++){
  //     force[i] = cosq[i]*(SUM___1[j]) - sinq[i]*(SUM___2[j]);
  //   }
  // }
  // Inverse Fourier Transform
  fftw_free(inpSin);
...
{
   double alpha        = 1.0;
   const int Npart     = 16384; //32768; //1048576;
   double *theta;
   double *force;
   theta = (double*) malloc(sizeof(double)*Npart);
   force = (double*) malloc(sizeof(double)*Npart);
   ...
   cosq = (double*) malloc(sizeof(double)*Npart);
   sinq = (double*) malloc(sizeof(double)*Npart);
